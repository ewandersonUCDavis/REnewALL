...
pricepath <- "D:\\Projects\\ReNewALL\\Data\\Nodal_Price_Actuals\\NODAL_PRICE_ACTUALS.csv"
...
old_price <- price_out
#################################
input_threshold=-14 ## CHOOSE THRESHOLD HERE ##
#################################

price_out$sign <- replace(price_out[,3],which(price_out[,3] > input_threshold),1)
price_out$sign <- replace(price_out[,4],which(price_out[,4] < input_threshold),-1)


## FOR THRESHOLD OF -$14/MWH
for (n in c(1:length(price_out[,1]))) {
  if (price_out$price[n] > input_threshold) {
    avg <- mean(price_out[which(price_out[,3] > input_threshold),3])
    stdev <- sd(price_out[which(price_out[,3] > input_threshold),3])
    high <- avg+stdev
    low <- input_threshold
    price_out$sign[n] <- ( (price_out$price[n]-input_threshold) / (high - input_threshold) )
  } else {
    avg <- mean(price_out[which(price_out[,3] < input_threshold),3])
    stdev <- sd(price_out[which(price_out[,3] < input_threshold),3])
    low <- avg-stdev
    high <- input_threshold
    price_out$sign[n] <- ( (price_out$price[n]-low) / (input_threshold-low) )-1
  }
}
...
load(file = 'D:\\Projects\\ReNewALL\\Scripts\\Functions\\CreateANNInputData_NhrsWindow.rda')
...
model <- h2o.deeplearning(x = 2:ncolshex,    
                          ...
                          stopping_metric="MSE",
                          model_id="DeepLearning_model_R_1_w_thresh")
h2o.saveModel(object = model, path = paste("SavedANN\\",sep=""), force = TRUE)
model <- h2o.deeplearning(x = 2:ncolshex,    
                          y = 1,
                          training_frame=train_hexfinal,
                          validation_frame=test_hexfinal,
                          activation = "Rectifier",
                          hidden = c(50,100,50),
                          epochs = 100000,
                          variable_importances=T,
                          ...
                          stopping_tolerance=0.001,
                          stopping_metric="MSE",
                          model_id="DeepLearning_model_R_2_w_thresh")
h2o.saveModel(object = model, path = paste("SavedANN\\",sep=""), force = TRUE)
model <- h2o.deeplearning(x = 2:ncolshex,    
                          y = 1,
                          training_frame=train_hexfinal,
                          ...
                          stopping_tolerance=0.001,
                          stopping_metric="MSE",
                          model_id="DeepLearning_model_R_3_w_thresh")
h2o.saveModel(object = model, path = paste("SavedANN\\",sep=""), force = TRUE)
model <- h2o.deeplearning(x = 2:ncolshex,    
                          y = 1,
                          training_frame=train_hexfinal,
                          ...
                          stopping_tolerance=0.001,
                          stopping_metric="MSE",
                          model_id="DeepLearning_model_R_4_w_thresh")
h2o.saveModel(object = model, path = paste("SavedANN\\",sep=""), force = TRUE)
model <- h2o.deeplearning(x = 2:ncolshex,    
                          y = 1,
                          training_frame=train_hexfinal,
                          ...
                          stopping_tolerance=0.001,
                          stopping_metric="MSE",
                          model_id="DeepLearning_model_R_5_w_thresh")
h2o.saveModel(object = model, path = paste("SavedANN\\",sep=""), force = TRUE)
