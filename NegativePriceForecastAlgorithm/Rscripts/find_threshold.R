...
for (n in c(1:length(dts))) {
  ...
  forc_path <- paste("Price_Forecast_",format(dts[n],format="%Y%m%d_%H00"),".csv",sep="")
...
threshold_file <- "curr_threshold.txt"

old_threshold_file <- paste("thresholds\\threshold_",format(start_date,format="%Y%m%d_%H00"),".txt",sep="")
